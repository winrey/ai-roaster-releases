...
The currently recommended release is the [0.2.5 stable release](https://github.com/winrey/ai-roaster-releases/releases/tag/v0.2.5). See this release's `BUILD-INFO.txt` / `NOTARIZATION.json` for signing, notarization, and verification results, and `SHA256SUMS.txt` for file checksums. See each earlier release for its own verification status. The app has not yet been tested on a physical Intel Mac.
